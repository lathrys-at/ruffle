//! Configuration: per-channel registration and the fusion knobs (§4, §5, §6, §8).
//!
//! Later algorithm modules read these structs; they do not edit this file. Every knob
//! has a conservative default chosen so the shipped behaviour stays close to plain
//! RRF, the recall-safe baseline. The defaults err toward keeping channels and
//! tilting weights only mildly (§3).
...
pub struct DiscriminationConfig {
    ...
    /// computed. Below it the bulk is too degenerate for the ratio to mean anything.
    pub min_distinct_values: usize,
    /// Floors the separation statistic's denominator toward the inter-quartile gap by this
    /// fraction, so a near-tied bulk cannot blow the ratio up.
    ...
    /// Small positive floor on `g`, so an unsure channel still votes. Zeroing a channel on
    /// one noisy read is a recall risk.
    pub g_floor: f64,
    ...
    /// sharply to a departure from the channel's norm; the conservative default keeps the
    /// response gentle, so one query cannot swing a channel hard.
    pub g_slope: f64,
}
...
pub struct CouplingConfig {
    ...
    /// on trades a slice of recall-safety for precision.
    ...
    /// zero between-refresh variance by construction and so passes the stability gate
    /// without demonstrating anything. Requiring at least two refreshes makes the
    /// stability gate an actual test rather than a vacuous one.
...
///
/// Off by default. Decay ties a merge to an external clock, which softens the exact merge
/// identity into an advertised approximation.
///
...
/// Reject `value` unless `ok` holds, naming the offending field.
...
impl FuseConfig {
    ...
    /// this before accepting a configuration, so an out-of-range knob is a loud
    /// construction-time error rather than a mid-query panic or a silent misweighting.
    /// It is public so a caller assembling a configuration from external input can
...
/// once at channel configuration, never per query. `good_score` is the optional declared
/// reference for the absolute-goodness statistic; when absent, that reference is learned
/// from early traffic and absolute goodness cold-starts.
...
impl ChannelConfig {
    /// Register a channel. Pass `good_score` as `Some` to enable absolute goodness from
    /// the first query, or `None` to learn the reference from traffic.
